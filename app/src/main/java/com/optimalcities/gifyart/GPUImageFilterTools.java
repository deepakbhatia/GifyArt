...
*/
    ...
    private static GPUImageFilter createBlendFilterStream(Context context, Class<? extends GPUImageTwoInputFilter> filterClass, InputStream imageStream) {
        try {
            GPUImageTwoInputFilter filter = filterClass.newInstance();
            filter.setBitmap(BitmapFactory.decodeStream(imageStream));
            return filter;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    private static GPUImageFilter createBlendFilter(Context context, Class<? extends GPUImageTwoInputFilter> filterClass) {
        try {
            GPUImageTwoInputFilter filter = filterClass.newInstance();
            filter.setBitmap(BitmapFactory.decodeResource(context.getResources(), android.R.color.white));
            return filter;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    private enum FilterTypeTool {

        SEPIA,PIXELATE, CROSSHATCH,EMBOSS,TOON,SWIRL,HUE

    }

    private enum PhotoEditTool {
        GRAYSCALE,BRIGHTNESS,COLOR_BALANCE,SOBEL_EDGE_DETECTION
    }
